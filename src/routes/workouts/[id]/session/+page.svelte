<script>
	...
	let currentExercise = workout?.exercises?.[currentIndex] ?? null;
	let totalExercises = workout?.exercises?.length ?? 0;
	let progressPercent = totalExercises > 0 ? ((currentIndex + 1) / totalExercises) * 100 : 0;

	function syncExerciseState() {
		currentExercise = workout?.exercises?.[currentIndex] ?? null;
...
<section class="container py-4 workout-session">
	...
	{:else if isFinished}
		<div
			class="card bg-dark border-secondary shadow-lg rounded-3 mx-auto"
		...
		>
			<div class="card-body p-4 p-md-5 text-center">
				...
				<p class="text-secondary mb-4">
					Great job! You've finished <strong class="text-light">{workout.name}</strong>.
				</p>
	...
	{:else if showBreakScreen}
		<div class="alert alert-warning border-0 shadow-sm rounded-3 mx-auto" style="max-width: 760px;">
			<div class="d-flex flex-column align-items-center text-center gap-3 py-3">
				...
				<div class="display-4 fw-bold text-dark bg-white rounded-3 px-4 py-2">
	...
	{:else if totalExercises === 0}
		<div
			class="card bg-dark border-secondary shadow-sm rounded-3 mx-auto"
			style="max-width: 760px;"
		>
			<div class="card-body p-5 text-center">
				...
				<a href={resolve('/workouts')} class="btn btn-outline-light">Back to Workouts</a>
			</div>
		</div>
		...
		<div class="card bg-dark border-secondary shadow-lg rounded-3 mb-4">
			<div class="card-body p-4 p-md-5 d-flex flex-column gap-4">
				...
				<div class="row g-3">
					{#if currentExercise.sets !== undefined}
						<div class="col-12 col-md-4">
							<div
								class="p-3 rounded-3 bg-black bg-opacity-25 border border-secondary h-100 text-center"
							>
				...
				{#if currentExercise.duration}
					<div
						class={`text-center p-4 rounded-3 border ${isTimeUp ? 'bg-success bg-opacity-10 border-success' : 'bg-black bg-opacity-25 border-secondary'}`}
					>
						<div class="display-1 fw-bold timer-display mb-2">{formatTime(timeRemaining)}</div>
						<div class="text-secondary mb-3">
							{isTimeUp ? "Time's Up!" : isPaused ? 'Paused' : 'Timer'}
						</div>
						{#if isTimeUp}
							<button class="btn btn-primary btn-orange" on:click={continueAfterTimeUp}
								>Continue</button
							>
						{/if}
					</div>
