...
function validateSession(body) {
	...
	if (!isValidObjectId(workoutId))
		errors.push('`workoutId` is required and must be a valid ObjectId string');
	if (!workoutName || typeof workoutName !== 'string')
		errors.push('`workoutName` is required and must be a string');
	if (!startedAt || Number.isNaN(Date.parse(startedAt)))
		errors.push('`startedAt` is required and must be a valid ISO date string');
	if (!completedAt || Number.isNaN(Date.parse(completedAt)))
		errors.push('`completedAt` is required and must be a valid ISO date string');
	if (exerciseCount !== undefined && (!Number.isFinite(exerciseCount) || exerciseCount < 0))
		errors.push('`exerciseCount` must be a non-negative number');
	if (plannedDuration !== undefined && (!Number.isFinite(plannedDuration) || plannedDuration < 0))
		errors.push('`plannedDuration` must be a non-negative number');
	if (
		actualDurationSeconds !== undefined &&
		(!Number.isFinite(actualDurationSeconds) || actualDurationSeconds < 0)
	)
		errors.push('`actualDurationSeconds` must be a non-negative number');
